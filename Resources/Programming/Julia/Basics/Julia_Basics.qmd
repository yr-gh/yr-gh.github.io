...
A function (inner function) **with references which are used locally, but defined in an enclosing scope (e.g. outer function)** is **defined inside another function (outer function)** and **returned by that function**. Then, when calling the outer function, the inner function will be returned with an environment, recording some state of outer function.

```{julia}
function make_pow(n::Real)  # Outer function
    function (x::Real)  # Inner function
        x^n  # The inner function uses n defined outside it
    end
end
...
Note: a `predicate` is a function that takes some value and always returns a Boolean value.
